...
async def test_api_verification_flow(real_db_client: AsyncClient):
    ...
    scenario_data = {
        ...
        "sequences": [
            {"id": "seq-1", "name": "Seq 1", "npcs": ["npc-1"], "enemies": ["enemy-1"]}
        ],
    ...
    inject_resp = await real_db_client.post(
        "/state/scenario/inject", json=scenario_data
    )
    ...
    details_resp = await real_db_client.get(
    ...
    trans_resp = await real_db_client.put(
        f"/state/session/{session_id}/sequence", json=trans_data
    )
